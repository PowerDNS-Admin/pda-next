...
class ConfigLoader:
    ...
    @staticmethod
    def load_file(path: Union[Path, str]) -> dict:
    ...
    @staticmethod
    def load_yaml(path: Union[Path, str]) -> Union[dict, list[dict], None]:
        """Loads the given configuration file and returns a dictionary of key/value pairs."""
...
class ConfigUtil:
    ...
    @staticmethod
    def merge(config: dict, update: dict) -> dict:
        """ Merges the given update dictionary into the given configuration dictionary. """
        ...
        for key, value in update.items():
            if key in config and isinstance(value, dict):
                config[key] = ConfigUtil.merge(config[key], value)
            elif key in config and isinstance(value, list):
